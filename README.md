...
2. Реализуем следующие компоненты:
    ...
    4. User - принимает отзыв и рисует информацию о нем;
...
4. Используем в комопненте User для отображения рейтинга отзыва.
...
1. Реализовать модули User, User по аналогии с Restaurant, Dish;
2. По аналогии с Menu отобразить Reviews;
3. В User отобразите пользователя(нужно сделать компонент и контейнер для него). UserContainer должен принимать userId, который есть в отзыве;
